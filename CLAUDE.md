...
**Link check (2026-09-06):** all 36 cam and 36 report URLs from Simon's sheet return HTTP 200 from a script; the 27 Surfline forecast URLs return 403 to scripts (bot block) but open in a browser. Cam links are copied one-to-one from the sheet, there is no selection logic.
