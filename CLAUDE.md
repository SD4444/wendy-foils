...
**Map view (added 2026-09-06).** Third way into the same data: `docs/map.html`, hosted at **https://sd4444.github.io/wendy-foils/map.html**, with a List / Map switch at the top of both surf pages. Built by `gen_surf_dashboard.py` on every run from the template `templates/surf_map.html` (the generator always writes `map.html` next to the `surf.html` it is given, so the workflow needs no extra step; its commit step adds `docs/map.html`). Leaflet 1.9.4 + Leaflet.markercluster 1.5.3 from cdnjs with SRI hashes. **Basemap: Esri World Dark Gray Canvas + Reference labels (free, attribution required, no key), with a "satellite" toggle to Esri World Imagery; if the Esri tiles error the page falls back to OpenStreetMap tiles darkened with a CSS filter.** CARTO dark tiles were tried first and now return "API KEY REQUIRED" watermark tiles, do not use them. Design: one pill per spot showing the day's face height (m), pill colour = verdict (go green, maybe amber, skip dark); overlapping spots cluster into a circle with the count and a ring split by verdict share; clusters dissolve at zoom 12; spot names appear from zoom 10; three live Candhis buoys as blue dots (reading shown from zoom 9). A day chip row (7 days, dots = how many go/maybe that day) recolours every marker. Tapping a spot opens a panel (right side on desktop, bottom sheet on phones, swipe down to close) with the same card as the list page plus a 7-day strip whose near days open the shared hourly chart modal. Deep links: `map.html#s=<spot n>&d=<day index>`. The map page's DATA adds `cards` (every spot x day, slimmed to per-day fields; static spot fields live once in `groups`) and `buoys`; the hourly chart JS and modal CSS are lifted verbatim from the list template at build time (`render_map()`), so there is one copy of that code. Size ~650 KB raw, ~70 KB gzipped.

**Browser test suite:** `tests/map_e2e.py` (Playwright, see its docstring for the venv setup). 53 checks on desktop 1400x900, iPhone-size 390x844 with touch, 768x1024 and 1024x700: markers/clusters render, all 36 spots inside the initial view, cluster click zooms, pills + names at zoom 13, panel opens for the clicked spot with 4 links and a 7-day strip, day chips (mouse, touch, keyboard), strip cell opens the chart, Esc closes chart then panel, close buttons, deep link, wheel zoom, drag pan, touch pan, no horizontal overflow on phones, sheet anchored to the bottom with the selected spot visible above it, modal above the sheet, tap targets >= 38 px, no console errors, no failed requests (cancelled tile loads excluded). Run it after touching the template or the generator; it saves screenshots to `tests/shots/`. Bugs it caught on 2026-09-06: the shared JS pulled in the list page's table listeners (null addEventListener), the chart modal rendered under the bottom sheet (z-index), the top bar's nowrap text widened the page grid to 509 px on phones (fixed with `grid-template-columns:minmax(0,1fr)` + `min-width:0`), Escape closed chart and panel in one press.
...
- Tide: per-spot `tide_pref`, shown on every card and matrix row as a plain label. From surf-forecast.com spot guides (fetched 2026-09-05): `any` for Carcans, Lacanau, Le Porge, La Jenny, Grand Crohot, Truc Vert, La Garonne, Biscarrosse, Cap de l'Homy, Saint-Girons, Messanges, Le Penon, Estagnots; `low` Hourtin and Cote des Basques (-1 at high when the beach disappears); `mid-high` La Piste/Santocha ("best around high"); `incoming` Graviere (Hossegor guides). Rule of thumb where the guide had no tide field or 404'd: `mid` for the other Medoc/Landes beach breaks, `mid-high` for the sheltered corners (Le Prevent, Chambre d'Amour, Grande Plage). Scoring: +0.5 in the preferred band (+0.7 for low/incoming), `any` +0.2 flat.
- Verdict: GO >= 5.5, MAYBE >= 3.5, else SKIP. Face floor overrides everything.
- Wave size is the **consensus mean of up to four wave models**: Open-Meteo best_match (MFWAM), `ecmwf_wam025`, `ncep_gfswave025`, and since 2026-09-06 the **Copernicus Marine IBI forecast** (see below). The swell partition is scaled with the consensus before the breaker formula. Confidence = model spread: <= 0.3 m "high (N wave models agree)", else "low". Days 4-7 "far out". **Gotcha fixed 2026-09-06:** GFS-Wave's 0.25 deg grid puts 18 of the 36 beaches on land and returns 0.0 all week; those zeros were averaged in and pulled the mean down ~25% while inflating the spread to 1-2 m. Any alt-model reading <= 0.05 m is now treated as missing.

**Live buoys + same-day bias check (added 2026-09-05).** Candhis (Cerema national wave-buoy network) has three live buoys on this coast: **Cap Ferret 03302**, **Anglet 06402**, **Saint-Jean-de-Luz 06403** (Hs, Hmax, period, direction, water temp, every 30-60 min). Candhis' own API needs a key by email (candhis@cerema.fr); we read them keyless through `https://thesurfkit.com/api/v2/buoys/nearest?lat=..&lng=..` (public endpoint of labouee.app, returns `last_reading`; the list/readings endpoints need an invite-only key). `fetch_buoys()` takes each buoy's latest reading, pulls EVERY wave model's Hs at the buoy for the same hour (best_match, ECMWF-WAM, GFS-Wave, Copernicus IBI via the `buoys` block of `data/copernicus.json`), and computes `bias = consensus mean / buoy`, i.e. the same quantity the spots are sized with (until 2026-09-06 it was best_match alone, which no longer matched what was displayed). Each day's entry in `data/buoy_bias.json` (committed by the Action) is `{bias, models: {model: ratio}}`; `bias_rolling` = median of the last 3 days. **The per-model ratios are the accuracy record**: after a week or two of trip mornings they say which model actually tracks this coast, and the consensus can be weighted accordingly. Do not claim any model is "more accurate" without reading that file. First two days: 2026-09-05 all models read x1.4-1.5 HIGH (small short-period day); 2026-09-06 they read LOW (Anglet buoy 1.7 m: best_match x0.86, ECMWF-WAM x0.93, IBI x0.77; SJDL 1.5 m: x0.92 / x1.05 / GFS x0.97 / IBI x0.89). So far the 3 km IBI is not obviously better than the coarser models; it is kept as one equal-weight member. Today's face heights for spots mapped to that buoy (`buoy` field: Gironde + Landes north of Vieux-Boucau -> Cap Ferret, the rest -> Anglet) are multiplied by `1/bias_rolling` clamped to 0.75-1.25, tomorrow by its square root, and the why/dashboard show "live buoy X 0.7m @ 7s, models read high x1.4, sized down today". On 2026-09-05 the model read x1.3-1.7 high vs all three buoys (small short-period day). Readings older than 3 h are ignored.
...
**Copernicus Marine IBI wave forecast (wired 2026-09-06).** Product `IBI_ANALYSISFORECAST_WAV_005_005`, dataset `cmems_mod_ibi_wav_anfc_0.027deg_PT1H-i`: MFWAM at 1/36 deg (~3 km), hourly, assimilates altimeter + CFOSAT, forced by ECMWF hourly wind and IBI currents, updated twice daily. The best free wave forecast for this coast. `fetch_copernicus.py` pulls ONE subset covering all spots (7 days, vars VHM0, VTPK, VTM10, VMDR, swell-1 partition, wind sea), snaps each spot to its nearest ocean cell (all within ~1-5 km; land cells are NaN), converts times to Europe/Paris and writes `data/copernicus.json` (~270 KB, committed). `surf.py` stays pure stdlib: `load_copernicus()` reads that file (ignored if older than 18 h) and feeds the Hs series into the consensus as model `copernicus_ibi`; each grid row also carries `ibi` (window-mean IBI Hs). Needs the `copernicusmarine` toolbox (pip) and a free account. **Credentials:** env `COPERNICUS_USERNAME` / `COPERNICUS_PASSWORD`. Locally they live in `.dev.vars` in the repo root (gitignored, KEY=value lines, read by the fetcher); in the Action they are repo secrets of the same name (uploaded 2026-09-06 with `gh secret set -f .dev.vars -R SD4444/wendy-foils`, run as SD4444). The workflow step "Copernicus IBI wave forecast" runs before the engine on daily and surf runs with `continue-on-error`, so if Copernicus is down the surf side falls back to the three Open-Meteo models. Local run: `pip install copernicusmarine` into a venv, then `python3 fetch_copernicus.py` followed by `python3 surf.py`. Only Hs is used so far; the IBI period and partition fields are in the JSON if the breaker formula should ever move to them. The JSON also carries the IBI series at the three Candhis buoy positions (`buoys` block) for the daily accuracy check. **Is it more accurate?** Unproven. Its resolution and data assimilation argue yes; the first buoy check (2026-09-06) had it reading 10-25% low while ECMWF-WAM was closest. Equal weight for now, let `data/buoy_bias.json` decide.

**Webcams (researched 2026-09-06, replaces the sheet's cam column).** Simon asked for the best camera per spot that actually shows that beach. Five research agents swept every provider (Viewsurf and its pv.viewsurf.com players, GoSurf, Skaping, Barrel Surfing, Quiksilver, Allosurf, surf-report, surf-forecast, Surf Sentinel, Windy, Windfinder, town and tourist-office sites, surf clubs, YouTube) and VERIFIED each candidate by fetching the page, reading the embed title, and downloading the current frame / checking its Last-Modified header. Result lives in `CAMS` in `surf.py` (merged into `SPOTS` at import): `cam` (best URL), `cam_shows` (what the frame shows and how it was verified), `cam_type` (live video vs stills and cadence), `cam_status`, `cam_dedicated` (False = no camera points at this beach; `cam` is then the nearest working OCEAN cam and `cam_km` its distance), `cam_alts` (up to 3 other angles/fallbacks). All 73 URLs returned HTTP 200 on 2026-09-06. The dashboard shows the button as "cam" or "nearest cam, N km", the description as a tooltip, and a "Cam:" / "No cam here:" line with the alternates on the today card and the section cards.
- **24 spots have a dedicated camera, 12 do not.** No camera exists anywhere for: L'Amelie, Le Gurp, Pin Sec, Hourtin-Plage (ocean), Carcans-Plage (ocean), La Jenny, La Garonne, L'Horizon/Les Dunes/La Pointe, Cap de l'Homy, Saint-Girons/Lette Blanche, Messanges, Tarnos. Their `cam` is the nearest ocean cam, labelled as such.
- **Sheet errors found:** Hourtin (Viewsurf 11106) and Carcans (Viewsurf 1255) were the LAKE beaches on Lac d'Hourtin-Carcans (flat water, confirmed from the frames), and Pin Sec pointed at that Hourtin lake cam. Lacanau's GoSurf page had a dead stream host. Messanges' and Vieux-Boucau's landesatlantiquesud pages contain no camera embed at all. Tarnos' surf-report page is a no-webcam placeholder. The tvcapferret page only links out to the Barrel Surfing cams.
- **Feeds down at check time:** Soulac (frozen since 27 Aug 2026 on every endpoint; Montalivet 15 km south is the fallback), all four La Salie Viewsurf angles ("Maintenance en cours" since at least 2 Sep; Biscarrosse 15 km south is the fallback). Moliets stills can be 15 h old overnight. Bourdaines/Estagnots share one PTZ camera (Estagnots is an hourly preset still). Hossegor has only two physical cameras and the providers disagree on which peak each shows, so no Graviere or La Nord view is promised.
- Almost everything on this coast is a Viewsurf camera; GoSurf, Quiksilver, Hastea, tourist offices etc. re-host the same streams. Prefer the direct `pv.viewsurf.com/<id>/<name>` player (loads fast, no lazy-load) or the official town page. Surfline has no cams anywhere on this coast; Windy webcam IDs are mostly dead (404).
- Report links stay as in the sheet (all HTTP 200); the 27 Surfline forecast links return 403 to scripts but open in a browser.
